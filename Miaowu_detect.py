...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	# previous frames, then compute the difference between the 
	...
	if text == "Occupied":
		...
		if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
			...
			# consistent motion is high enough
			if motionCounter >= conf["min_motion_frames"]:
				# check to see if dropbox sohuld be used
				currenttime=time.strftime(ISOTIMEFORMAT,time.localtime(time.time()))
				print currenttime
				path ="{base_path}/{timestamp}.jpg".format(base_path=conf["dropbox_base_path"],timestamp=currenttime)
				cv2.imwrite(path, frame)
				print "[UPLOAD] {}".format(ts)
				os.system("python /usr/bin/bypy.py syncup /home/pi/Raspi/video")
